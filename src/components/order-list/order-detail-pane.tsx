    ...
    <section className="erp-card flex min-h-[640px] flex-col overflow-hidden">
      ...
      <div className="flex flex-wrap items-start justify-between gap-2 border-b border-slate-200 px-3 py-2.5">
        <div className="min-w-0">
          {/* V112c: mã đơn co lại (truncate) để hai nhãn Loại đơn / Trạng thái luôn nằm 1 hàng. */}
          <div className="flex items-center gap-2">
            <Link className="min-w-0 truncate text-[15px] font-bold text-cyan-700 hover:underline" href={`/orders/${order.id}`}>{order.orderCode}</Link>
...
function StatusBadge({ orderType, status }: { orderType: string | null | undefined; status: string }) {
  ...
  return (
    <span className="inline-flex shrink-0 items-center justify-end gap-1">
      <span className={`inline-flex items-center whitespace-nowrap rounded-full border px-2 py-0.5 text-[10.5px] font-bold ${typeStyle}`}>
        {orderTypeLabel(orderType)}
      </span>
      <span className={`inline-flex items-center whitespace-nowrap rounded-full border px-2 py-0.5 text-[10.5px] font-bold ${statusStyle}`}>
        ● {statusLabel}
      </span>
    </span>
  );
