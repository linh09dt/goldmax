...
export function ShippingCalculator({ orders, initialRates, ratesPersisted, initialMappings, knownTenhangs }: Props) {
  ...
  }, [selectedOrder, rates, mappings, region, deliveryKm, mountainDistrict]);

  const rateModelOptions = useMemo(() => {
    const byCode = new Map<string, string>();
    for (const row of rates) {
      const code = row.modelCode.trim().toUpperCase();
      if (code && !byCode.has(code)) byCode.set(code, row.modelName);
    }
  ...
  async function saveMappings() {
    ...
    setNotice("");
    try {
      const response = await fetch("/api/shipping-model-mappings", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
                ...
                {mappings.length ? mappings.map((row, index) => {
                  ...
                  return (
                    <tr key={row.id ?? `mapping-${index}`}>
                      ...
                      <td className="border border-slate-200 px-2 py-2 text-center"><input type="checkbox" checked={row.active !== false} onChange={(event) => updateMapping(index, "active", event.target.checked, setMappings)} /></td>
...
function updateMapping<K extends keyof ShippingModelMappingRow>(index: number, key: K, value: ShippingModelMappingRow[K], setMappings: Dispatch<SetStateAction<ShippingModelMappingRow[]>>) {
  setMappings((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, [key]: value } : row));
}
