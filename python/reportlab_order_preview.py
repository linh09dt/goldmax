...
def _bottom_section(order: dict[str, Any]) -> Table:
    ...
    # V61: hộp tiền trải hết chiều ngang (nhãn trái – số tiền phải), chỉ kẻ vạch ngang giữa các dòng.
    summary = Table(summary_rows, colWidths=[CONTENT_W * 0.6, CONTENT_W * 0.4], hAlign="LEFT")
    commands: list[tuple[Any, ...]] = [
        ("BOX", (0, 0), (-1, due_index), 0.8, BORDER),
        ("LINEBELOW", (0, 0), (-1, due_index), 0.45, BORDER),
        ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
        ("LEFTPADDING", (0, 0), (-1, -1), 5),
        ("RIGHTPADDING", (0, 0), (-1, -1), 5),
    ...
    # V55/V61: hộp ghi chú nằm dưới box tiền (cách 1 dòng); cả hai trải hết chiều ngang.
